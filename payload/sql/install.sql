-- OpenTenBase adaptation of PGMQ SQL
-- Upstream: https://github.com/pgmq/pgmq
-- Upstream commit studied: 02cffd7
-- Upstream license: PostgreSQL License, copyright (c) 2023, Tembo
-- OpenTenBase compatibility changes in this file:
--   1. Removed metadata foreign keys rejected by OpenTenBase distribution rules.
...
CREATE UNLOGGED TABLE IF NOT EXISTS pgmq.notify_insert_throttle (
    queue_name           VARCHAR UNIQUE NOT NULL, -- Queue name (without 'q_' prefix); OTB: FK removed for distribution compatibility
...
(
    ...
    queue_name     text NOT NULL,  -- Name of the queue that receives messages when pattern matches; OTB: FK removed for distribution compatibility
    bound_at       TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, -- Timestamp when the binding was created
    compiled_regex text, -- OTB: generated stored column removed; topic routing is not part of MVP
...
CREATE INDEX IF NOT EXISTS idx_topic_bindings_covering ON pgmq.topic_bindings (pattern);
...
BEGIN
    EXECUTE FORMAT('SELECT msg_id, read_ct, enqueued_at, last_read_at, vt, message, headers FROM pgmq.%I WHERE msg_id = $1 FOR UPDATE', qtable)
    USING msg_id
    INTO msg;

    IF msg.msg_id IS NULL THEN
        RETURN false;
    END IF;

    EXECUTE FORMAT('INSERT INTO pgmq.%I (msg_id, vt, read_ct, enqueued_at, last_read_at, message, headers) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (msg_id) DO NOTHING RETURNING msg_id', atable)
    USING msg.msg_id, msg.vt, msg.read_ct, msg.enqueued_at, msg.last_read_at, msg.message, msg.headers
    INTO moved;
    ...
    EXECUTE FORMAT('DELETE FROM pgmq.%I WHERE msg_id = $1', qtable)
    USING msg.msg_id;
...
DECLARE
    id BIGINT;
BEGIN
    FOREACH id IN ARRAY msg_ids LOOP
        IF pgmq.archive(queue_name, id) THEN
            RETURN NEXT id;
        END IF;
    END LOOP;
    RETURN;
...
$$;
CREATE OR REPLACE FUNCTION pgmq.version()
RETURNS text
LANGUAGE SQL
IMMUTABLE
AS $$
    SELECT '1.11.2-otb.0'::text;
$$;
